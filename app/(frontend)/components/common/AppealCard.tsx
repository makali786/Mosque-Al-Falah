...
export default function AppealCard({
  ...
  const target = funding.goal || 0;
  const isOngoing = funding.isOngoing || false;
  const progressPercentage =
    !isOngoing && target > 0 ? Math.min((raised / target) * 100, 100) : 0;
  ...
  return (
    <div
      className={`bg-white rounded-[15px] overflow-hidden flex flex-col lg:max-w-89.25 shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)] ${className}`}
    >
      {/* Image Section */}
      <Link
        href={links.details || '/appeals'}
        className="relative w-full h-58.5 overflow-hidden block"
      >
        {imageUrl ? (
      ...
      <div className="flex flex-col gap-6 sm:px-5 pt-4 pb-9 px-4">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm text-[#71717A]">{organization.name}</p>
          </div>
