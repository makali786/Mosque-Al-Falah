...
const StandardServiceDetail = async ({
  service,
  params,
}: {
  service: any;
  ...
  const bannerDescription =
    service.detailBanner?.bannerSubtitle ||
    service.shortDescription ||
    extractTextFromRichText(service.fullDescription) ||
    '';
  ...
  const liveStreamUrl =
    service.media?.liveStreamUrl || (isLive ? service.media?.videoUrl : null);
  ...
  const targetDateStr =
    bannerCountdown?.enableCountdown && bannerCountdown.countdownTargetTime
  ...
  const countdownLabel =
    bannerCountdown?.enableCountdown && bannerCountdown.countdownLabel
  ...
  if (
    service.registration?.enableRegistration &&
    service.registration?.registrationButtonText
  ) {
    primaryButton = {
  ...
  return (
    <div>
      ...
      <EventMediaSection
        ...
        donationAmounts={
          service.donation?.suggestedAmounts?.map((a: any) => a.amount) || []
        }
          ...
          description={
            <>
              ...
              {blocks.eidTitle[0]?.congratulationsBlock?.description && (
                <RichTextRenderer
                  content={blocks.eidTitle[0].congratulationsBlock.description}
                />
              )}
      ...
      <ServiceQuote
        ...
        testimonials={
          service.testimonials?.map((t: any) => ({
            text: t.quote,
      ...
      <AboutQuoteSection
        ...
        donateButtonUrl={
          servicesPage?.bottomQuote?.donateButtonUrl || '/donate'
        }
      />
    </div>
  );
