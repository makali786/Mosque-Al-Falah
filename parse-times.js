...
for (let line of lines) {
  ...
  if (match) {
    ...
    if (timeTokens.length >= 11) {
      ...
      const jMaghrib =
        timeTokens[9] === '"' ? lastJamaat.maghrib : timeTokens[9];
      ...
      let mADhuhr;
      const dhuhrHour = parseInt(aDhuhr.split(':')[0]);
      if (dhuhrHour === 12 || dhuhrHour < 5) {
        mADhuhr = parseTimeToMinutes(aDhuhr, true);
      } else {
        mADhuhr = parseTimeToMinutes(aDhuhr, false);
      }

      const mAAsr = parseTimeToMinutes(aAsr, true);
      const mAMaghrib = parseTimeToMinutes(aMaghrib, true);
      const mAMaghribAdhan = parseTimeToMinutes(aMaghrib, true); // Some use this for Adhan check
      const mAIsha = parseTimeToMinutes(aIsha, true);

      // Convert Jamaat to minutes
      const mJFajr = parseTimeToMinutes(jFajr, false);
      // Dhuhr Jamaat is almost always PM (1:00, 1:30 etc)
      let mJDhuhr;
      const dhuhrJHour = parseInt(jDhuhr.split(':')[0]);
      if (dhuhrJHour === 12 || dhuhrJHour < 5) {
        mJDhuhr = parseTimeToMinutes(jDhuhr, true);
      } else {
        mJDhuhr = parseTimeToMinutes(jDhuhr, false);
      }
      ...
      const dateObj = new Date(
        Date.UTC(currentYear, currentMonth, parseInt(dateNum))
      );
